...
async function SignupUsecase(
  ...
  try {
    ...
    const isEmailRegistered = await executeQuery(
      `SELECT * FROM ecommerce.user_details where email = '${requestBody.email}'`
    );

    if (isEmailRegistered.length > 0) {
      throw new BadRequestException(
        `User with the ${requestBody.email} email is registered`
      );
    }
    ...
    }', '${UserRole.ADMIN.name}', '${requestBody.userName}', '${
